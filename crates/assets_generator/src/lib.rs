...
pub fn build_assets_enum(dest_path: &Path, src_path: Option<&Path>, name: Option<String>) {
    ...
    writeln!(f, "// This file is autogenerated").unwrap();
    writeln!(f, "use assets_helper::AssetsTrait;").unwrap();
    writeln!(f).unwrap();
    ...
    writeln!(f, "impl AssetsTrait for {name} {{").unwrap();
    writeln!(f, "    fn path(&self) -> &'static str {{").unwrap();
